...
.withMessage('Must be between 4 and 50 characters')
 ,
requirePrice:check('price')
.trim()
.toFloat()
.isFloat({min:1})
.withMessage('Must be a number greator than 1'),
...
requirePassword:check('password').trim().isLength({min:4,max:20}).withMessage('Must be beteween 4 and 20 characters'),
...
requiresPasswordForUser:check('password').trim().custom(async(password,{req})=>{
